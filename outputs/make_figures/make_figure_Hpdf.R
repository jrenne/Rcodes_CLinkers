...
a[vX]<-1
  ...
  all.Probas.Q <- foreach(i = 1:length(values.of.sl), 
                          .combine=rbind) %dopar% {
                            b    <- values.of.sl[i]
  ...
  all.Probas.P <- foreach(i = 1:H, .combine=rbind) %dopar% {
    Temperature.P <- fourier(model_sol,x,values.of.sl,i,vX)
    Temperature.P
  }
  ...
  nb.sl <- 200                                                          
  scale.sl.values <- seq(values.of.sl[1],
                         tail(values.of.sl,1),
                         length.out = nb.sl+1)
  all.pdf.Q <- NULL
  all.cdf.Q <- NULL
  ...
  for(t in 1:H){
    ...
    sl.Q <- splinefun(x=values.of.sl, y=cdf.Q, method="hyman")
    sl.P <- splinefun(x=values.of.sl, y=cdf.P, method="hyman")
    
    fitted.cdf.values.Q <- sl.Q(scale.sl.values)
    fitted.pdf.values.Q <- diff(fitted.cdf.values.Q)
    fitted.cdf.values.P <- sl.P(scale.sl.values)
  ...
  SLplot<-list(all.cdf.P,all.cdf.Q,all.CI.P,all.CI.Q,all.pdf.P,all.pdf.Q,
               nb.sl,scale.sl.values)
  save(SLplot,file="outputs/backup_parallel/pdf_sea_level.Rdat")


# Compute expected trajectories (under P and Q):
ES.P    <- EV$EX$H[1:H]
ES.Q    <- varphi.tilde(model_sol,a,H)[[1]]/
...
plot(model_sol$vec_date[2:(H+1)],ES.P,
     xlim=x.lim,ylim=y.lim, cex.main=1.5,cex.axis=1.5,cex.lab=1.5,
     col="white",xlab="",ylab="meter",las=1,
     main="(a) - Trajectory of global sea level rise")
lines(model_sol$vec_date[2:(H+1)],
      ES.P,lwd=2,col=P.col.line)
...
legend("topright",
       ...
       lty=c(2,2),
       col=c("lightsteelblue4","grey28"),cex=1.5,
       lwd=c(2,2),bty = "n")
for(i in length(vector.of.CI):1){
  # P
  polygon(c(model_sol$vec_date[2:(H+1)],rev(model_sol$vec_date[2:(H+1)])),
          scale.sl.values[c(all.CI.P[1,,i],rev(all.CI.P[2,,i]))],
          col=P.col,border = NA)
}

plot(model_sol$vec_date[2:(H+1)],ES.Q,
     xlim=x.lim,ylim=y.lim,las=1,
     col="white",xlab="",ylab="meter", cex.main=1.5,cex.axis=1.5,
     cex.lab=1.5,
     main="(b) - Trajectory of global sea level rise including Risk-Premium")
for(i in length(vector.of.CI):1){
  # Q
  polygon(c(model_sol$vec_date[2:(H+1)],rev(model_sol$vec_date[2:(H+1)])),
          scale.sl.values[c(all.CI.Q[1,,i],rev(all.CI.Q[2,,i]))],
          col=Q.col,border = NA)
}
lines(model_sol$vec_date[2:(H+1)],
      ES.Q,lwd=2,col=Q.col.line)
lines(model_sol$vec_date[2:(H+1)],
      ES.P,lwd=2,col=P.col.line)

plot(scale.sl.values[2:(nb.sl+1)],all.pdf.P[,H],type="l",
     col=P.col.line,lwd=3,xlim=c(min(values.of.sl)+0.2,
                                 max(values.of.sl)-0.2),
     main="(c) - P.d.f. of global sea level rise in 2100",
     cex.main=1.5,cex.axis=1.5,cex.lab=1.5,
     xlab="",ylab="",yaxt="no")
lines(scale.sl.values[2:(nb.sl+1)],all.pdf.Q[,H],
